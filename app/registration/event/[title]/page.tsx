...
export default function EventRegistrationPage({ params }: PageProps) {
  ...
  // Initialize input fields array
  const [participants, setParticipants] = useState<ParticipantInput[]>([]);
  const [error, setError] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isLoadingAuth, setIsLoadingAuth] = useState(true);

  // Set initial participant fields and check session
  useEffect(() => {
    // Check session by fetching current registrations
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(`Successfully registered for ${event?.title || eventTitle}!`);
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <div className="flex-grow pt-32 pb-24 px-6 max-w-2xl mx-auto w-full flex flex-col">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            >
              {isSubmitting ? "PROCESSING TRANSMISSION..." : "SUBMIT REGISTRATION"}
            </button>
          </form>
        </div>
