...
function CountdownTimer() {
  ...
  return (
    <div className="flex items-center justify-center gap-3 sm:gap-6 md:gap-8 bg-slate-950/40 px-6 py-5 rounded-3xl border border-cyan-500/10 backdrop-blur-xl shadow-[0_15px_35px_rgba(0,0,0,0.5)]">
      {segments.map((seg, i) => (
        <div key={seg.label} className="flex items-center gap-3 sm:gap-6 md:gap-8">
            ...
            <div className="relative overflow-hidden h-14 sm:h-20 md:h-28 flex items-center justify-center">
              <AnimatePresence mode="popLayout">
                <motion.span
                  ...
                  className="font-orbitron text-4xl sm:text-6xl md:text-8xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white via-cyan-100 to-cyan-400 drop-shadow-[0_0_20px_rgba(0,242,254,0.45)] tabular-nums"
                >
                  {seg.value}
                </motion.span>
              </AnimatePresence>
          ...
          {i < segments.length - 1 && (
            <motion.span
              ...
              className="font-orbitron text-2xl sm:text-4xl md:text-6xl font-black text-cyan-400/60 self-start mt-2 sm:mt-4 md:mt-6"
            >
              :
            </motion.span>
          )}
